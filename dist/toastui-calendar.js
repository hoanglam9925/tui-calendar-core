...
function createStoreContext() {
  ...
  const useStore = function (selector) {
    ...
    const hasErrorRef = hooks_module_A(false); // `null` can be a valid state slice.
...
const LayoutContainerContext = R(null);
...
const FloatingLayerContext = R(null);
...
function Layout(_ref) {
  ...
  const layoutClassName = hooks_module_T(() => "".concat(cls('layout'), " ").concat(className), [className]);
  hooks_module_(() => {
    if (container) {
...
function useTimeGridEventResize(_ref) {
  ...
  const oneRowHeight = hooks_module_T(() => baseResizingInfo ? timeGridData.rows[0].height : 0, [baseResizingInfo, timeGridData.rows]); // When drag an one-day event
...
function useInterval(callback, delay) {
  const savedCallback = hooks_module_A(callback); // Remember the latest callback.
...
function useCalendarData(calendar) {
  ...
  return hooks_module_T(() => ({ ...calendar,
